...
public class BoardPosition
{
	...
	int piece, i;
	i = 1;
	if (-1 <= WHITE_KingFile-file && WHITE_KingFile-file <= 1 &&
	    -1 <= WHITE_KingRank-rank && WHITE_KingRank-rank <= 1)
	    return false;
	while (rank + i <= 8) // forward
		...
		if (piece==6) piece=0; // kings do not attack!
	...
	int piece, i;
	i = 1;
	if (-1 <= BLACK_KingFile-file && BLACK_KingFile-file <= 1 &&
	    -1 <= BLACK_KingRank-rank && BLACK_KingRank-rank <= 1)
	    return false;
	while (rank + i <= 8)
	    {
		piece = AT[file][rank + i];
    ...
    public void AddThisMove (long v)
    {
	long c = 0; // is_check
	...
	if (!WTM && AttacksBlack (WHITE_KingFile, WHITE_KingRank))
	    c = 1;
	move_list[COUNT_OF_LEGAL_MOVES++] = v | (c << 63);
    }
    ...
    public void AddMove (int from_file, int from_rank,
			 int to_file, int to_rank, int type,long tok)
    {
	long v = (from_file - 1) + ((from_rank - 1) << 3) +
	    ((to_file - 1) << 6) + ((to_rank - 1) << 9) + (type << 12);
	v |= tok;
	if (WTM && AT[to_file][to_rank] == 1 && from_rank == 7)
	    {
		for (int i = 5; i >= 2; i--) // promotion
		    {
	...
	// for capture promotions, AT[to] is 0, and so it is ignored
	// in a perfect world, PGNs would have a similar decision..
    ...
    {
	int captured = AT[to_file][to_rank];
	int mover = AT[from_file][from_rank];
	if (Chess960 && type == 1)
	    {
		...
		AT[7][1] = 6;
		// fixme 960
		AddMove (Chess960_WK_File, 1, White_KR_file, 1, type,0);
		AT[6][1] = 0;
		AT[7][1] = 0;
	...
	if (Chess960 && type == 2)
		...
		AT[3][1] = 6;
		// fixme 960
		AddMove (Chess960_WK_File, 1, White_QR_file, 1, type,0);
		AT[4][1] = 0;
		AT[3][1] = 0;
		AT[Chess960_WK_File][1] = 6;
	...
	if (type>=3) // if cap, explodes my king, not legal
	    {
		if (-1 <= WHITE_KingFile-to_file
		    && WHITE_KingFile-to_file <= 1
		    && -1 <= WHITE_KingRank-to_rank
		    && WHITE_KingRank-to_rank <= 1) return;
	    }	
	...
	if (type>=3)
		...
		v=(long) AT[to_file-1][to_rank-1]; // -1,-1
		if (v!=1 && v!=-1 && v!=0)
		{tok|=((v+8)<<31); AT[to_file-1][to_rank-1]=0;}
		v=(long) AT[to_file-1][to_rank-0]; // -1,0
		if (v!=1 && v!=-1 && v!=0)
		{tok|=((v+8)<<35); AT[to_file-1][to_rank-0]=0;}
		v=(long) AT[to_file-1][to_rank+1]; // -1,+1
		if (v!=1 && v!=-1 && v!=0)
		{tok|=((v+8)<<39); AT[to_file-1][to_rank+1]=0;}
		v=(long) AT[to_file-0][to_rank-1]; // 0,-1
		if (v!=1 && v!=-1 && v!=0)
		{tok|=((v+8)<<43); AT[to_file-0][to_rank-1]=0;}
		v=(long) AT[to_file-0][to_rank+1]; // 0,1
		if (v!=1 && v!=-1 && v!=0)
		{tok|=((v+8)<<47); AT[to_file+0][to_rank+1]=0;}
		v=(long) AT[to_file+1][to_rank-1]; // 1,-1
		if (v!=1 && v!=-1 && v!=0)
		{tok|=((v+8)<<51); AT[to_file+1][to_rank-1]=0;}
		v=(long) AT[to_file+1][to_rank-0]; // 1,0
		if (v!=1 && v!=-1 && v!=0)
		{tok|=((v+8)<<55); AT[to_file+1][to_rank+0]=0;}
		v=(long) AT[to_file+1][to_rank+1]; // 1,1
		if (v!=1 && v!=-1 && v!=0)
		{tok|=((v+8)<<59); AT[to_file+1][to_rank+1]=0;}
	    }	
	
	if (type==28) AT[to_file][to_rank-1]=0; // ep
	if (type>=3 && // if is cap, explodes opp king, legal
	     -1 <= BLACK_KingFile-to_file && BLACK_KingFile-to_file <= 1
	    && -1 <= BLACK_KingRank-to_rank && BLACK_KingRank-to_rank <= 1)
	    AddMove(from_file,from_rank,to_file,to_rank,type,tok);
	else
	    {
		if (mover == 6)
		{
		    WHITE_KingFile=to_file; WHITE_KingRank=to_rank;
		    if (!AttacksBlack (to_file, to_rank))
			AddMove (from_file,from_rank,to_file,to_rank,type,tok);
		    WHITE_KingFile=from_file; WHITE_KingRank=from_rank;
		}
		else
		{
	...
	if (type>=3) // unexplode
	{
	    w=(int) ((tok>>31)&15); if (w!=0) AT[to_file-1][to_rank-1]=w-8;
	    w=(int) ((tok>>35)&15); if (w!=0) AT[to_file-1][to_rank-0]=w-8;
	    w=(int) ((tok>>39)&15); if (w!=0) AT[to_file-1][to_rank+1]=w-8;
	    w=(int) ((tok>>43)&15); if (w!=0) AT[to_file-0][to_rank-1]=w-8;
	    w=(int) ((tok>>47)&15); if (w!=0) AT[to_file-0][to_rank+1]=w-8;
	    w=(int) ((tok>>51)&15); if (w!=0) AT[to_file+1][to_rank-1]=w-8;
	    w=(int) ((tok>>55)&15); if (w!=0) AT[to_file+1][to_rank-0]=w-8;
	    w=(int) ((tok>>59)&15); if (w!=0) AT[to_file+1][to_rank+1]=w-8;
	}
	AT[from_file][from_rank] = mover;
	AT[to_file][to_rank] = captured;
	if (type == 28)
	...
	if (Chess960 && type == 1)
		...
		AT[7][8] = -6;
		// fix me 960
		AddMove (Chess960_BK_File, 8, Black_KR_file, 8, type,0);
		AT[6][8] = 0;
		AT[7][8] = 0;
	...
	if (Chess960 && type == 2)
		...
		AT[3][8] = -6;
		// fixme 960
		AddMove (Chess960_BK_File, 8, Black_QR_file, 8, type,0);
		AT[3][8] = 0;
		AT[4][8] = 0;
		AT[Chess960_BK_File][8] = -6;
	...
	if (!Chess960 && type == 2)
        ...
        if (type>=3)     // if cap, explodes my king, not legal
            {
                if (-1 <= BLACK_KingFile-to_file
                    && BLACK_KingFile-to_file <= 1
                    && -1 <= BLACK_KingRank-to_rank
                    && BLACK_KingRank-to_rank <= 1) return;
            }
	AT[to_file][to_rank] = AT[from_file][from_rank];
	AT[from_file][from_rank] = 0;
	...
	if (type==28)
        ...
        if (type>=3 && // if is cap, explodes opp king, legal
             -1 <= WHITE_KingFile-to_file && WHITE_KingFile-to_file <= 1
	     && -1 <= WHITE_KingRank-to_rank && WHITE_KingRank-to_rank <= 1)
            AddMove(from_file,from_rank,to_file,to_rank,type,tok);
	else
	    {
		if (mover == -6)
		{
                    BLACK_KingFile=to_file; BLACK_KingRank=to_rank;
		    if (!AttacksWhite (to_file, to_rank))
			AddMove (from_file,from_rank,to_file,to_rank,type,tok);
                    BLACK_KingFile=from_file; BLACK_KingRank=from_rank;
		}
		else
		{
	...
	if (WTM)
		...
		if (AT[to_file][to_rank] < 0 && AT[to_file][to_rank] != -6
		    && AT[from_file][from_rank]!=6)
		    LegalWhite (from_file, from_rank, to_file, to_rank,
				-5+2+5*AT[from_file][from_rank]-AT[to_file][to_rank]);
		return true;
	    }
	else
	    {
		...
		if (AT[to_file][to_rank] > 0 && AT[to_file][to_rank] != 6
		    && AT[from_file][from_rank]!=-6)
		    LegalBlack (from_file, from_rank, to_file, to_rank,
				-5+2-5*AT[from_file][from_rank]+AT[to_file][to_rank]);
		return true;
	    }
    }
	...
	if (piece == 1)
		...
		if (WTM)
			...
			if (file < 8 && AT[file + 1][rank + 1] < 0 && AT[file + 1][rank + 1] != -6)
			    LegalWhite (file, rank, file + 1, rank + 1, 2+0-AT[file + 1][rank + 1]);
		    ...
		    {
			if (file < 8 && AT[file + 1][rank - 1] > 0 && AT[file + 1][rank - 1] != 6)
			    LegalBlack (file, rank, file + 1,rank - 1, 2+0+AT[file + 1][rank - 1]);
			if (file > 1 && AT[file - 1][rank - 1] > 0 && AT[file - 1][rank - 1] != 6)
			    LegalBlack (file, rank, file - 1,rank - 1, 2+0+AT[file - 1][rank - 1]);
			if (AT[file][rank - 1] == 0)
	...
	if (WhiteOOO && AT[White_QR_file][1] == 4 &&
		...
		if (b)
		    LegalWhite (Chess960_WK_File, 1, White_QR_file, 1, 2);
	    }	    
	if (WhiteOO && AT[White_KR_file][1] == 4 &&
		...
		if (b)
		    LegalWhite (Chess960_WK_File, 1, White_KR_file, 1, 1);
	    }	    
    }
	...
	if (BlackOOO && AT[Black_QR_file][8] == -4 &&
		...
		if (b)
		    LegalBlack (Chess960_BK_File, 1, Black_QR_file, 1, 2);
	    }	    
	if (BlackOO && AT[Black_KR_file][8] == -4 &&
		...
		if (b)
		    LegalBlack (Chess960_BK_File, 1, Black_KR_file, 1, 1);
	    }	    
    }
	...
	if (EnPassant != 0)
		...
		if (WTM)
			...
			if (EnPassant < 8 && AT[EnPassant + 1][5] == 1)
			    LegalWhite (EnPassant + 1, 5, EnPassant, 6, 28);
			...
			if (EnPassant < 8 && AT[EnPassant + 1][4] == -1)
			    LegalBlack (EnPassant + 1, 4, EnPassant, 3, 28);
 ...
 1)
	...
	for (int i = 0; i < COUNT_OF_LEGAL_MOVES; i++)
	    {
		lmove = move_list[i];
		type = (int) (lmove >> 12) & 31;
		check = (int) (lmove >> 63) & 1;
		prom = (int) (lmove >> 17) & 7;
		move = (int) (lmove & 0xfffff);
		S = "";
		if (type == 1)
		    S = "O-O";
		else if (type == 2)
	...
	for (int i = 0; i < COUNT_OF_LEGAL_MOVES; i++)
		...
		return i;
	String T; // be nice with checks
	if (S.indexOf("+")==-1) T=S+"+";
	else T=S.substring(0,S.length()-1);
	for (int i = 0; i < COUNT_OF_LEGAL_MOVES; i++)
	    if (T.equals (move_list_annotated[i]))
		return i;
	if (S.indexOf("x")!=-1 && S.indexOf("=")!=-1) // capture promotion
	    {
		T=S.substring(0,4);
		for (int i = 0; i < COUNT_OF_LEGAL_MOVES; i++)
		    if (T.equals (move_list_annotated[i]))
	...
	if (WTM)
		...
		AT[to_file][6] = 1;
		AT[to_file][5] = 0;
		AT[to_file][6] = 0; // explode
		int w;
		w=AT[to_file-1][6-1]; if (w!=1 && w!=-1) AT[to_file-1][6-1]=0;
		w=AT[to_file-0][6-1]; if (w!=1 && w!=-1) AT[to_file-0][6-1]=0;
		w=AT[to_file+1][6-1]; if (w!=1 && w!=-1) AT[to_file+1][6-1]=0;
		w=AT[to_file-1][6-0]; if (w!=1 && w!=-1) AT[to_file-1][6-0]=0;
		w=AT[to_file+1][6-0]; if (w!=1 && w!=-1) AT[to_file+1][6-0]=0;
		w=AT[to_file-1][6+1]; if (w!=1 && w!=-1) AT[to_file-1][6+1]=0;
		w=AT[to_file+0][6+1]; if (w!=1 && w!=-1) AT[to_file+0][6+1]=0;
		w=AT[to_file+1][6+1]; if (w!=1 && w!=-1) AT[to_file+1][6+1]=0;
		...
		AT[to_file][3] = -1;
		AT[to_file][4] = 0;
		AT[to_file][3] = 0; // explode
		int w;
		w=AT[to_file-1][3-1]; if (w!=1 && w!=-1) AT[to_file-1][3-1]=0;
		w=AT[to_file-0][3-1]; if (w!=1 && w!=-1) AT[to_file-0][3-1]=0;
		w=AT[to_file+1][3-1]; if (w!=1 && w!=-1) AT[to_file+1][3-1]=0;
		w=AT[to_file-1][3-0]; if (w!=1 && w!=-1) AT[to_file-1][3-0]=0;
		w=AT[to_file+1][3-0]; if (w!=1 && w!=-1) AT[to_file+1][3-0]=0;
		w=AT[to_file-1][3+1]; if (w!=1 && w!=-1) AT[to_file-1][3+1]=0;
		w=AT[to_file+0][3+1]; if (w!=1 && w!=-1) AT[to_file+0][3+1]=0;
		w=AT[to_file+1][3+1]; if (w!=1 && w!=-1) AT[to_file+1][3+1]=0;
    ...
    public String GetDirect (long lMOVE)
    {
	int MOVE = (int) (lMOVE & 0x7fffffff);
        int mv = MOVE & 0x1ffff;
...
////////////////////////////////////////////////////////////////
	...
	if (MOVE_TREE != null && MOVE_TREE.NOW.MainLineParent != null)
	    {
		int c = (int) MOVE_TREE.NOW.MainLineParent.mainline.move;
		LAST_FROM_x = 1 + (c & 07);
    ...
    public void UnMakeMove32 (long lmv)
    {
	int mv = (int) (lmv & 0x7fffffff);
	int move = mv & 07777;
	int from_file = 1 + (move & 7), from_rank = 1 + ((move >> 3) & 7),
	    to_file = 1 + ((move >> 6) & 7), to_rank = 1 + ((move >> 9) & 7);
	int piece = AT[to_file][to_rank]; // hmm, does not exist when cap...
	int type = (mv >> 12) & 31;
	int prom = (mv >> 17) & 7;
	int OoOoO = (mv >> 20) & 0xf;
	int ct = (mv >> 24) & 127;
	...
	if (type == 1)
		...
		if (type >= 3 && type <= 27)
		{
		    int x=type-3; // map 0 to 24
		    int pie=x/5,cap=x-(5*pie); pie++; cap++;
		    AT[to_file][to_rank] = WTM ? -cap : cap;
		    AT[from_file][from_rank] = WTM ? pie : -pie;
		}
		if (type == 28) // unmake EP
		    {
			AT[from_file][from_rank] = WTM ? 1 : -1;
			if (WTM)
		...
		if (type >=3) // unexplode
		{
		  int w; long v=lmv; // unexplode
		  w=(int) ((v>>31)&15); if (w!=0) AT[to_file-1][to_rank-1]=w-8;
		  w=(int) ((v>>35)&15); if (w!=0) AT[to_file-1][to_rank-0]=w-8;
		  w=(int) ((v>>39)&15); if (w!=0) AT[to_file-1][to_rank+1]=w-8;
		  w=(int) ((v>>43)&15); if (w!=0) AT[to_file-0][to_rank-1]=w-8;
		  w=(int) ((v>>47)&15); if (w!=0) AT[to_file-0][to_rank+1]=w-8;
		  w=(int) ((v>>51)&15); if (w!=0) AT[to_file+1][to_rank-1]=w-8;
		  w=(int) ((v>>55)&15); if (w!=0) AT[to_file+1][to_rank-0]=w-8;
		  w=(int) ((v>>59)&15); if (w!=0) AT[to_file+1][to_rank+1]=w-8;
		}
		if (piece == 6)
		    {
    ...
    public void MakeMove32 (long lmv, String S)
    {
	int mv = (int) (lmv & 0x7fffffff);
	int move = mv & 0x1ffff;
	int prom = (mv >> 17) & 7;
	...
	boolean oo = (((move >> 12) & 31) == 1);
	boolean ooo = (((move >> 12) & 31) == 2);
	boolean ep = (((move >> 12) & 31) == 28);
	int type = (move >> 12) & 31;
	...
	if (MOVE_TREE != null)
	    MOVE_TREE.NodeMove (lmv | (ct << 24) | (OoOoO << 20), S);
	...
	if (type>=3) // do explosion
	{
	    int w;
	    AT[to_file][to_rank]=0;
	    w=AT[to_file-1][to_rank-1];
	    if (w!=1 && w!=-1) AT[to_file-1][to_rank-1]=0;
	    w=AT[to_file-0][to_rank-1];
	    if (w!=1 && w!=-1) AT[to_file-0][to_rank-1]=0;
	    w=AT[to_file+1][to_rank-1];
	    if (w!=1 && w!=-1) AT[to_file+1][to_rank-1]=0;
	    w=AT[to_file-1][to_rank-0];
	    if (w!=1 && w!=-1) AT[to_file-1][to_rank-0]=0;
	    w=AT[to_file+1][to_rank-0];
	    if (w!=1 && w!=-1) AT[to_file+1][to_rank-0]=0;
	    w=AT[to_file-1][to_rank+1];
	    if (w!=1 && w!=-1) AT[to_file-1][to_rank+1]=0;
	    ...
	    w=AT[to_file+1][to_rank+1];
	    if (w!=1 && w!=-1) AT[to_file+1][to_rank+1]=0;		
	    // exploding the rook at home square negates OO permissions
	    // somehow this is OK in the FEN, but not in the thruput to engine
	    if (to_file<=2 && to_rank<=2) WhiteOOO=false; // fixme 960
	    if (to_file>=7 && to_rank<=2) WhiteOO=false; // fixme 960
	    if (to_file<=2 && to_rank>=7) BlackOOO=false; // fixme 960
	    if (to_file>=7 && to_rank>=7) BlackOO=false; // fixme 960
	}
	WTM = !WTM;
	if (WTM)
	    MOVE_NUMBER++;	
...
// 0-11 to-from, 12-16 type, 17-19 prom, 20-23 prior oo
// 24-30 prior rev count w/eptarg, 31-62 explosions, 63 check
// types 1-2 are castling, 3-27 are captures (5*5), type 28 is ep
